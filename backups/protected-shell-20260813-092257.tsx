...
export function ProtectedShell({
  ...
  const navItems: NavItem[] = [
    { label: "Panel principal", href: "/protected", icon: "home" },
    {
      label: "Crear contenido",
      href: has("content.create") ? "/protected/create" : undefined,
      icon: "plus",
      denied: !has("content.create"),
    },
    {
      label: "Biblioteca",
      href: has("content.view") ? "/protected/library" : undefined,
      icon: "library",
      denied: !has("content.view"),
    },
    { label: "Publicaciones", icon: "send", disabled: true },
    { label: "Calendario", icon: "calendar", disabled: true },
    { label: "Estadísticas", icon: "chart", disabled: true },
    {
      label: "Usuarios y permisos",
      href: has("users.view") ? "/protected/users" : undefined,
      icon: "users",
  ...
  const nav = (
    <>
      <div className="flex h-20 items-center gap-3 border-b border-white/10 px-6">
        ...
        <div>
          <p className="text-base font-semibold tracking-tight text-white">ContentAI</p>
          <p className="text-xs text-slate-400">Content Workspace</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        ...
        <nav className="space-y-1.5">
          {navItems.map((item) => {
            ...
            if (item.disabled || item.denied || !item.href) {
              return (
                <div
                  key={item.label}
                ...
                >
                  ...
                  <span className="rounded-md border border-white/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-slate-600">
                    {item.denied ? "Sin acceso" : "Próximo"}
                  </span>
                </div>
              );
            }
      ...
      <div className="min-h-screen lg:pl-72">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">Plataforma privada</p>
              <p className="hidden text-xs text-slate-500 sm:block">Creación y gestión de contenidos</p>
            </div>
          </div>
